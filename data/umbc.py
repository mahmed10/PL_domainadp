...
class UMBC(torch.utils.data.Dataset):
	def __init__(self, path_list, train = 'train', data_mode ='rgb', scale = 1, target_width = 572, target_height = 572):
		...
		with open("./dataset/UMBC/umbc_copy.yaml", 'r') as stream:
	...
	def supdata(self, index):
		...
		return torch.as_tensor(img.copy()).float().contiguous(),torch.as_tensor(mask.copy()).long().contiguous(),img_path
